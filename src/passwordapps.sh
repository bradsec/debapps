...
function install_opt_app() {
    ...
    if [[ "${app}" == "bitwarden" ]]; then
        ...
        latest_release=$(curl -s "https://github.com/bitwarden/clients/releases/" | grep -o '<a[^>]*href="/bitwarden/clients/releases/tag/desktop-v[^"]*"' | head -n 1 | awk -F '"' '{print $2}')
        ...
        local latest_release
        latest_release=$(curl -s "https://github.com/keepassxreboot/keepassxc/releases" | grep -o '<a[^>]*href="/keepassxreboot/keepassxc/releases/tag/[^"]*"' | head -n 1 | awk -F '"' '{print $2}')
        local version_number
