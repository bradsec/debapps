...
print_message() {
    ...
    case "${option}" in
        ...
        BLANK)
            ...
            ;;
        DONE)
            format="${border_sym}${BOLD}${GREEN}[${done_sym}]${RESET} %s\n"
            ;;
        PASS)
            format="${border_sym}${GREEN}[${pass_sym}]${RESET} %s\n"
            ;;
        *)
            format="%s"
            ;;
    ...
    # Format is selected from the fixed case list above.
...
function menu_select() {
    ...
    if [[ ${options_count} -eq 0 ]]; then
        print_message FAIL "No menu options supplied."
        return 1
    fi
    ...
    if ! { true < "${tty}"; } 2>/dev/null || ! { true > "${tty}"; } 2>/dev/null; then
        {
            ...
            echo
            printf " Enter option [1-%d]: " "${options_count}"
        } >&2
        read -r key
        echo "${key}"
        return 0
    ...
    clear > "${tty}"
    {
        if declare -F debapps_header >/dev/null 2>&1; then
            debapps_header with_system
        fi
        echo
        echo -e " ${title}"
        echo
        echo " Use arrow keys or j/k. Press Enter to select."
        echo
    } > "${tty}"
    draw_menu_options
    ...
    while true; do
        ...
        elif [[ "${selected}" -ge "${options_count}" ]]; then
            selected=0
        fi
...
function remove_opt_app() {
    ...
    if [[ -z "${app_name}" || ! "${app_name}" =~ ^[a-zA-Z0-9._-]+$ ]]; then
        print_message FAIL "Invalid /opt application name: ${app_name}"
        return 1
    fi

    app_command=$(echo "${app_name}" | tr '[:upper:]' '[:lower:]')
    user_name=$(get_user)
    user_home=$(getent passwd "${user_name}" | cut -d: -f6)

    run_command rm -rf "/opt/${app_name}"
    run_command rm -f "/usr/local/bin/${app_command}" "/usr/sbin/${app_command}"
    run_command rm -f "/usr/share/applications/${app_command}.desktop"

    if [[ -n "${user_home}" && -d "${user_home}/.local/share/applications" ]]; then
        run_command rm -f "${user_home}/.local/share/applications/${app_command}.desktop"
        run_command rm -f "${user_home}/.local/share/applications/start-${app_command}.desktop"
    fi
...
function download_file() {
    ...
    if wget --user-agent=Mozilla --content-disposition -c -E -O \
        ...
        if wget --user-agent=Mozilla --content-disposition -E -O \
            ...
            echo
            print_message DONE "File successfully downloaded."
        elif command -v curl >/dev/null 2>&1 && curl -L -J "${src_url}" -o "${dst_file}" --progress-bar; then
            echo
            print_message DONE "File successfully downloaded."
        else
...
function download_content() {
	...
	local output
	if output=$(wget -qO- "${src_url}"); then
		echo "${output}"
	else
