...
setup_app_image() {
    ...
    if [[ -z ${app_image_name_lower} ]]; then
        ...
        run_command ln -sf "${app_image_path}" "/usr/local/bin/${app_image_name_lower}"
        echo "/usr/local/bin/${app_image_name_lower}" >> "${install_log_file}"
    fi

    print_message INFO "Extracting AppImage to find icon image and desktop config..."
    cd "${app_image_dir}" || exit
    ...
    desktop_config=$(find "${app_image_dir}" -type f -name "*.desktop" -print -quit | sed 's|^\./||')

    if [[ -n "${desktop_config}" && -e "${desktop_config}" ]]; then
        ...
        local clean_desktop_config_filename
        clean_desktop_config_filename=${desktop_config_filename//[^[:alnum:]._-]/}
        ...
        local icon_image
        icon_image=$(find "$app_image_dir" -type f -name "*${icon_name}*.png" -print -quit | sed 's|^\./||')
    else
    ...
    if [[ -n "${icon_image}" && -e "${icon_image}" ]]; then
        ...
        for size in "${icon_sizes[@]}"; do
            ...
            sized_icon_image=$(find "$app_image_dir" -type f -wholename "*/${size}/${icon_name}.png" -print -quit | sed 's|^\./||')
            local target_path="${icon_dir}/${size}/apps"
            local target_file="${target_path}/${icon_name}.png"

            if [[ -n "${sized_icon_image}" && -e "${sized_icon_image}" ]]; then
        ...
        if command -v update-desktop-database >/dev/null 2>&1; then
            print_message INFO "Refreshing desktop application database..."
    ...
    # shellcheck disable=SC2154 # app variable passed from calling function
...
remove_app_image() {
    ...
    if [[ "${log_file}" != /opt/*.log && "${log_file}" != /opt/*/install.log ]]; then
    ...
    local app_image_dir
    app_image_dir=$(dirname "${first_line}")
    ...
    while IFS= read -r line; do
        ...
        if [[ -z "$line" || "$line" =~ \.\. || ! "$line" =~ ^(/opt/|/usr/share/icons/hicolor/|/usr/share/applications/|/usr/sbin/|/usr/local/bin/) ]]; then
